...
data = cell(length(data_file_name));
time = cell(length(data_file_name));
obs_posi = cell(length(data_file_name));
tar_posi = cell(length(data_file_name));
ee_posi = cell(length(data_file_name));
...
for i=1:length(data_file_name)
    ...
    grid on
    disp("Error Average : ")
    disp(mean(row_norm(tar_posi{i}-ee_posi{i})))
...
function distance = getSD(pose, obs_posi, mesh)
    ...
    [distance, pt_closest] = point2trimesh('Faces',f,...
                                           'Vertices',v,...
